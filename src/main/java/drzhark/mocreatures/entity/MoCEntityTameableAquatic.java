...
public class MoCEntityTameableAquatic extends MoCEntityAquatic implements IMoCTameable {
    ...
    protected static final DataParameter<Optional<UUID>> OWNER_UNIQUE_ID = EntityDataManager.defineId(MoCEntityTameableAquatic.class, DataSerializers.OPTIONAL_UUID);
    protected static final DataParameter<Integer> PET_ID = EntityDataManager.<Integer>defineId(MoCEntityTameableAquatic.class, DataSerializers.INT);
    protected static final DataParameter<Boolean> TAMED = EntityDataManager.<Boolean>defineId(MoCEntityTameableAquatic.class, DataSerializers.BOOLEAN);
    ...
    public Boolean processTameInteract(PlayerEntity player, Hand hand) {
        ...
        //stores in fishnet
        if (!stack.isEmpty() && stack.getItem() == MoCItems.FISHNET && stack.getDamageValue() == 0 && this.canBeTrappedInNet()) {
            if (!this.level.isClientSide) {
